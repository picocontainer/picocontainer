...
/**
 * A {@link com.picocontainer.ComponentMonitor} which writes to a Log4J {@link org.apache.log4j.Logger} instance.
...
public class Log4JComponentMonitor implements ComponentMonitor, Serializable {
    ...
    public <T> void instantiated(final PicoContainer container, final ComponentAdapter<T> componentAdapter,
        ...
        Logger logger = getLogger(constructor);
        if (logger.isDebugEnabled()) {
            logger.debug(format(ComponentMonitorHelper.INSTANTIATED, ctorToString(constructor), duration,
    ...
    public <T> void instantiationFailed(final PicoContainer container,
        ...
        if (logger.isEnabledFor(Level.WARN)) {
            logger.warn(format(ComponentMonitorHelper.INSTANTIATION_FAILED, ctorToString(constructor), 
    ...
    public void lifecycleInvocationFailed(final MutablePicoContainer container,
        ...
        if (logger.isEnabledFor(Level.WARN)) {
            logger.warn(format(ComponentMonitorHelper.LIFECYCLE_INVOCATION_FAILED, memberToString(method), instance, cause != null ? cause.getMessage() : "null"), cause);
        }
